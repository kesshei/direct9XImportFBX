...
//
//      MessageBox(nullptr,(const WCHAR*)pCompileErrors, L"Compile Error",MB_OK | MB_ICONEXCLAMATION);
//
//      assert(0);
//   }
//}
...
void SingleFbxMesh::_loadModel()
{
   ...
   assert(importerPtr->Initialize(m_filename.c_str(), -1, m_sdkManagerPtr->GetIOSettings()));
...
// Non-vital info, but FYI!
  ...
  // ÓÐÎÊÌâ assert(m_scenePtr->GetSrcObjectCount<FbxConstraint>() == 0);
...
// causes memory leaks
   ...
   if (SceneSystemUnit.GetScaleFactor() != 1.0)
   {
       ...
       FbxSystemUnit::cm.ConvertScene(m_scenePtr);
   }
